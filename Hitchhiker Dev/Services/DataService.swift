...
//  Hitchhiker Dev
//
//  Created by rotem.sade on 30/09/2018.
...
class DataService {
    ...
    private var _REF_USERS = DB_BASE.child(FB_USERS)
    private var _REF_DRIVERS = DB_BASE.child(FB_DRIVERS)
    private var _REF_TRIPS = DB_BASE.child(FB_TRIPS)
    ...
    func passengerIsOnTrip(passengerKey: String, handler: @escaping(_ status: Bool?, _ driverKey: String?, _ tripKey: String?) -> Void) {
        DataService.instance._REF_TRIPS.observe( .value, with: { (tripSnapshot) in
            if let tripSnapshot = tripSnapshot.children.allObjects as? [DataSnapshot] {
                for trip in tripSnapshot {
