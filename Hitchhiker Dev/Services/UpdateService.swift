...
class UpdateService {
                    ...
                    if user.key == Auth.auth().currentUser?.uid {
                        if !user.hasChild(USER_IS_DRIVER) {
                            if let userDict = user.value as? Dictionary<String, AnyObject> {
                                ...
                                let destinationArray = userDict[TRIP_COORDINATES] as? NSArray
    ...
    func cancelTrip(withPassengerKey passengerKey: String, forDriverKey driverKey: String?) {
        ...
        DataService.instance.REF_USERS.child(passengerKey).child(TRIP_COORDINATES).removeValue()
        if let driverKey = driverKey {
